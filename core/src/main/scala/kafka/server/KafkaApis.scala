...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      trace(s"Handling request:${request.summary.getDescription(true)} from connection ${request.context.connectionId};" +
      ...
      request.summary.apiLocalCompleteTimeNanos = time.nanoseconds
    }
  }
  ...
  def handleProduceRequest(request: RequestChannel.Request) {
    ...
    val numBytesAppended = request.header.toStruct.sizeOf + request.summary.sizeInBytes
  ...
  private def updateRecordsProcessingStats(request: RequestChannel.Request, tp: TopicPartition,
    ...
    if (conversionCount > 0) {
      ...
      request.summary.messageConversionsTimeNanos = processingStats.conversionTimeNanos
    }
    request.summary.temporaryMemoryBytes = processingStats.temporaryMemoryBytes
  }
  ...
  private def closeConnection(request: RequestChannel.Request, errorCounts: java.util.Map[Errors, Integer]): Unit = {
    ...
    requestChannel.sendResponse(new RequestChannel.Response(request.summary, None, CloseConnectionAction, None))
  }
  ...
  private def sendResponse(request: RequestChannel.Request, responseOpt: Option[AbstractResponse]): Unit = {
    ...
    val requestSummary = request.summary
    responseOpt match {
      case Some(response) =>
