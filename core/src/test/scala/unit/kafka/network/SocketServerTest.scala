...
class SocketServerTest extends JUnitSuite {
  ...
  def processRequest(channel: RequestChannel, request: RequestChannel.Request) {
    ...
    channel.sendResponse(new RequestChannel.Response(request.summary, Some(send), SendAction, None))
  }
  ...
  def testGracefulClose() {
    ...
    for (_ <- 0 until 10) {
      ...
      server.requestChannel.sendResponse(new RequestChannel.Response(request.summary, None, RequestChannel.NoOpAction, None))
    }
  }
